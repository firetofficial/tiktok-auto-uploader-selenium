    ...
    def run_upload_videos(self, cookies, caption, wait_time, proxy, browser_name):
        ...
        while video_files:
            ...
            try:
                ...
                print(self.systemNoiti + Fore.CYAN + f"{browser_name}: " + Fore.WHITE + "Đợi đăng nhập/Waiting for login...")
                print("--------------------------------------------------------------------------------")
                print(Style.RESET_ALL)
                
            except:
                print(self.error + Fore.YELLOW + "Lỗi cookie/Cookie error!")
                print(Style.RESET_ALL)
            ...
            print(self.systemNoiti + Fore.CYAN + f"{browser_name}" + Fore.WHITE +  "Bắt đầu đăng video/Start posting videos!")
            ...
            for retry_count in range(MAX_RETRY):
                try:
                    ...
                    print(self.error + Fore.YELLOW + f"{browser_name}" + Fore.WHITE + "Tài khoản bị khoá chức năng đăng bài/Account has been locked from posting!")
                    print("--------------------------------------------------------------------------------")
            ...
            if not video_files:
                print(self.systemNoiti + Fore.CYAN + f"{browser_name}" + Fore.WHITE + "Đã hết video. Thoát chương trình/The video is over. Exit the program!")
                print(Style.RESET_ALL)
                break
            ...
            print(f"{self.systemNoiti}" + Fore.CYAN + f"{browser_name}: Đăng video/Post videos: {video}")
            print("--------------------------------------------------------------------------------")
            print(Style.RESET_ALL)
            ...
            try:
                ...
                post.click()
                print(self.systemNoiti + Fore.CYAN + f"{browser_name}" + Fore.WHITE + "Video đang được đăng, vui lòng đợi/Video is being posted, please wait!!")
                print("--------------------------------------------------------------------------------")
                print(Style.RESET_ALL)
            ...
            success_flag = True
            print(self.systemNoiti + Fore.CYAN + f"{browser_name}" + Fore.WHITE + f"Đã đăng video/Video posted: {video}")
            print("--------------------------------------------------------------------------------")
            print(Style.RESET_ALL)
            ...
            try:
                ...
                print("--------------------------------------------------------------------------------")
                print(self.systemNoiti + Fore.CYAN + f"{browser_name}" + Fore.WHITE + f"Đợi {wait_time} giây trước khi tiếp tục/Wait {wait_time} seconds before continuing...")
                print(Style.RESET_ALL)
            except:
                wait_time_int = int(wait_time)
                minute_time = wait_time_int * 60
                print("--------------------------------------------------------------------------------")
                print(self.systemNoiti + Fore.CYAN + f"{browser_name}" + Fore.WHITE + f"Đợi {wait_time_int} giây trước khi tiếp tục/Wait {wait_time} seconds before continuing...")
                print(Style.RESET_ALL)
